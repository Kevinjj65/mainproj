...
def get_translator(model_id: str):
    """
    Load NLLB tokenizer + model with proper device/dtype handling.
    ...
    try:
        ...
        if use_cuda:
            ...
            # CPU: use fp32 and standard loading
            model = AutoModelForSeq2SeqLM.from_pretrained(
                str(local_path),
                trust_remote_code=True,
        ...
        if "torchvision" in str(e) or "nms" in str(e):
            raise RuntimeError(
                f"Translator loading failed due to dependency conflict:\n{e}\n\n"
                "This is likely a torch/torchvision version mismatch.\n"
                "Try: pip install --upgrade transformers torch torchvision"
            ) from e
        raise
...
def translate(text: str, src_lang: str, tgt_lang: str, max_tokens: int = 256) -> str:
    ...
    gen_kwargs = {
        ...
        "use_cache": False,  # Avoid meta tensor issues
    }
